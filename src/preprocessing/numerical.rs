...
struct StandardScaler<T: RealNumber> {
    means: Vec<T>,
    stds: Vec<T>,
...
impl<T: RealNumber> StandardScaler<T> {
    ...
    /// standard-deviation should be kept. Otherwise, replace it by zero.
        ...
        Ok(build_matrix_from_columns(
            self.means
                ...
                .map(|(column_index, (column_mean, column_std))| {
                    take_column_from_matrix(x, column_index)
...
mod tests {
    ...
    mod helper_functionality {
        use super::super::{build_matrix_from_columns, ensure_std_valid, take_column_from_matrix};
        ...
        fn take_second_column_from_matrix() {
            ...
            let second_column = take_column_from_matrix(&four_columns, 1);
            assert_eq!(
                second_column,
